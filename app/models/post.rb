class Post < ApplicationRecord
  ...
  def self.sort(selection)
    ...
    when 'likes'
      return Post.find(Favorite.group(:post_id).order('count(post_id) desc').pluck(:post_id))
    when 'dislikes'
      return Post.find(Favorite.group(:post_id).order(Arel.sql('count(post_id) asc')).pluck(:post_id))
    end
  end
